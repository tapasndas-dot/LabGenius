...
class SampleRepository:
    ...
    def apply_filters(self, query, *, search=None, status=None, priority=None, **filters):
        if search:
            pattern = f"%{search.strip()}%"
            query = query.filter(or_(Sample.sample_number.ilike(pattern), Sample.external_reference.ilike(pattern), Sample.sample_description.ilike(pattern)))
        if status is not None: query = query.filter(Sample.status == status)
        if priority is not None: query = query.filter(Sample.priority == priority)
        for field, value in filters.items():
            if value is not None: query = query.filter(getattr(Sample, field) == value)
        return query
