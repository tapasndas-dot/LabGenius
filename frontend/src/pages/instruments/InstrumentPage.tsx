...
import { INSTRUMENT_CRITICALITIES, INSTRUMENT_STATUSES, instrumentsApi, type Instrument, type InstrumentInput, type StabilityChamberProfile, type StabilityChamberProfileInput } from '../../api/instruments'
...
function ChamberProfileForm({ profile, onSave, onCancel }: { profile: StabilityChamberProfile | null; onSave: (values: StabilityChamberProfileInput) => Promise<void>; onCancel: () => void }) {
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const data = new FormData(event.currentTarget); const decimal = (name: string) => { const raw = String(data.get(name) ?? '').trim(); return raw ? Number(raw) : null }; const optional = (name: string) => String(data.get(name) ?? '').trim() || null; await onSave({ temperature_setpoint: decimal('temperature_setpoint'), temperature_unit: optional('temperature_unit'), humidity_setpoint: decimal('humidity_setpoint'), humidity_unit: optional('humidity_unit'), description: optional('description') }) }
  return <form className="admin-form form-grid" onSubmit={submit}><h2>{profile ? 'Edit' : 'Create'} Stability Chamber Profile</h2><label>Temperature setpoint<input name="temperature_setpoint" type="number" step="any" defaultValue={profile?.temperature_setpoint ?? ''} /></label><label>Temperature unit<input name="temperature_unit" maxLength={20} defaultValue={profile?.temperature_unit ?? ''} /></label><label>Humidity setpoint<input name="humidity_setpoint" type="number" step="any" defaultValue={profile?.humidity_setpoint ?? ''} /></label><label>Humidity unit<input name="humidity_unit" maxLength={20} defaultValue={profile?.humidity_unit ?? ''} /></label><label className="full-width">Description<textarea name="description" defaultValue={profile?.description ?? ''} /></label><div className="form-actions full-width"><button>Save Profile</button><button type="button" className="text-button" onClick={onCancel}>Cancel</button></div></form>
}
...
export function InstrumentPage() {
  ...
  const [profileInstrument, setProfileInstrument] = useState<Instrument | null>(null); const [profile, setProfile] = useState<StabilityChamberProfile | null>(null); const [profileMissing, setProfileMissing] = useState(false); const [profileForm, setProfileForm] = useState(false); const [profileConflict, setProfileConflict] = useState(false)
  ...
  async function loadProfile(record: Instrument) { setProfileInstrument(record); setProfileForm(false); setProfileConflict(false); try { setProfile(await instrumentsApi.getChamberProfile(record.id)); setProfileMissing(false); setError(null) } catch (cause) { if (cause instanceof ApiError && cause.status === 404) { setProfile(null); setProfileMissing(true); setError(null) } else setError(errorMessage(cause)) } }
  async function saveProfile(values: StabilityChamberProfileInput) { if (!profileInstrument) return; setProfileConflict(false); try { if (profile) await instrumentsApi.updateChamberProfile(profileInstrument.id, profile.version, values); else await instrumentsApi.createChamberProfile(profileInstrument.id, values); setProfileForm(false); await loadProfile(profileInstrument) } catch (cause) { setProfileConflict(cause instanceof ApiError && cause.status === 409); setError(errorMessage(cause)) } }
